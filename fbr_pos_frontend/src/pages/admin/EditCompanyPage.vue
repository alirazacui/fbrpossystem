<template>
  <div class="space-y-6 pb-12">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">Edit Business</h1>
        ...
        <div class="space-y-6">
          <div>
            ...
            <input
              ...
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
            <p class="text-xs text-gray-500 mt-1">Optional. Upload new logo only if you want to replace current one.</p>
            <div v-if="logoPreview || form.logo" class="mt-3">
              <img :src="logoPreview || form.logo" alt="Logo preview" class="h-20 w-20 rounded-lg border border-gray-200 object-cover" />
            </div>
          </div>
...
onMounted(async () => {
  if (companyId) {
    try {
      ...
      // Ensure fbr_business_nature is an array for the multiselect checkboxes
      if (typeof company.fbr_business_nature === 'string') {
        company.fbr_business_nature = company.fbr_business_nature.split(',').map(s => s.trim());
      } else if (!Array.isArray(company.fbr_business_nature)) {
        company.fbr_business_nature = [];
      }
      form.value = { ...form.value, ...company };
    } catch (err: any) {
      error.value = "Failed to load company details for editing";
      console.error(err);
    } finally {
      loading.value = false;
    }
...
const submitForm = async () => {
  ...
  try {
    ...
    await companyAPI.updateCompany(companyId, payload)
    // Success - redirect back to details
    router.push(`/admin/tenants/${companyId}`)
  } catch (err: any) {
