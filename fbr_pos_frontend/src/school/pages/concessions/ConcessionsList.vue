<template>
  <div class="h-full flex flex-col px-8 py-8">
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Fee Concessions</h1>
        <p class="text-sm text-gray-500 mt-1">Manage discounts and fee concessions for students.</p>
      </div>
      <router-link to="/school/concessions/create" class="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-indigo-700 shadow-sm flex items-center gap-2">
    ...
    <div class="flex items-center gap-4 mb-6">
      <div class="relative flex-1 max-w-md">
        <input v-model="searchQuery" @keyup.enter="handleSearch" type="text" placeholder="Search by student or reason..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm" />
      ...
      <button @click="handleSearch" class="px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 shadow-sm">Search</button>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50 sticky top-0 z-10">
            <tr>
              ...
              <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Session</th>
              <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Fee Head</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else-if="items.length === 0">
              <td colspan="6" class="px-6 py-12 text-center text-gray-500">No concessions found.</td>
            </tr>
            <tr v-else v-for="item in items" :key="item.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.session_name || '—' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.fee_head_name || 'Global' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">{{ item.concession_type.replace('_', ' ') }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-indigo-600">
                <span v-if="item.concession_type === 'percentage'">{{ item.percentage }}%</span>
                <span v-else>Rs {{ item.amount }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <router-link :to="`/school/concessions/${item.id}`" class="text-indigo-600 hover:text-indigo-900 mr-4">View</router-link>
                <router-link :to="`/school/concessions/${item.id}/edit`" class="text-gray-600 hover:text-gray-900">Edit</router-link>
              </td>
            </tr>
          </tbody>
        </table>
...
const items = ref<StudentFeeConcession[]>([])
...
const fetchItems = async () => {
  ...
  try {
    ...
    const res = await studentFeeConcessionAPI.list(params)
