  ...
  <div class="h-full flex flex-col px-8 py-8">
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Manage and view student exam performance.</p>
      </div>
      <router-link to="/school/exam-results/create" class="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-indigo-700 shadow-sm flex items-center gap-2">
    ...
    <div class="flex items-center gap-4 mb-6">
      <div class="relative flex-1 max-w-md">
        <input v-model="searchQuery" @keyup.enter="handleSearch" type="text" placeholder="Search by student..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm" />
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50 sticky top-0 z-10">
            <tr>
              ...
              <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Exam</th>
              ...
              <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Grade</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else-if="items.length === 0">
              <td colspan="6" class="px-6 py-12 text-center text-gray-500">No results found.</td>
            </tr>
            <tr v-else v-for="item in items" :key="item.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.exam_name || '—' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.subject_name || '—' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-indigo-600">{{ item.marks_obtained }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.grade || '—' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <router-link :to="`/school/exam-results/${item.id}/edit`" class="text-gray-600 hover:text-gray-900">Edit</router-link>
              </td>
            </tr>
          </tbody>
        </table>
...
const items = ref<StudentExamResult[]>([])
...
const fetchItems = async () => {
  ...
  try {
    ...
    const res = await studentExamResultAPI.list(params)
