<template>
  <div class="p-8 max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
      ...
      <div><h1 class="text-2xl font-bold text-gray-900">Edit Link</h1><p class="text-sm text-gray-500">Update guardian relationship.</p></div>
    </div>
    ...
    <div v-else class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 space-y-5">
      ...
      <div>
        ...
        <input v-model="form.relation" type="text" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
      </div>
...
onMounted(async () => {
  try {
    const [linkRes, stuRes, grdRes] = await Promise.all([
      studentGuardianAPI.retrieve(linkId),
      studentAPI.list(),
      guardianAPI.list()
    ])
...
const handleSubmit = async () => {
  ...
  loading.value = true
  try {
    await studentGuardianAPI.update(linkId, form.value)
    ...
    serverError.value = data?.detail || 'Failed to update link.'
