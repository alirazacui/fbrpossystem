...
# ---------------------------------------------------------------------------
# WarehouseStock  — per-warehouse stock level for a product
# ---------------------------------------------------------------------------

class WarehouseStock(models.Model):
    """
    Stock level of a product within a specific warehouse.
    One record per (warehouse, product) pair.
    When module_warehousing is enabled, stock is tracked at warehouse level
    instead of the flat Product.current_stock field.
    """
    ...
    low_stock_threshold = models.DecimalField(
        ...
        validators=[MinValueValidator(0)],
        verbose_name=_("Low Stock Threshold"),
        help_text=_("Alert when quantity falls below this. Set 0 to disable."),
    )
    ...
    class Meta:
        ...
        verbose_name_plural = _("Warehouse Stock")
...
class Sale(models.Model):
    ...
    def _generate_sale_number(self) -> str:
        """
        Generates a globally unique, sequential invoice number.
        Format: INV-{company_id}-{YYYY}-{NNNNNN}

        Uses the company's database primary key (guaranteed unique across all
        companies) as a prefix, so two companies can never generate the same
        invoice number. The sequence resets per company per year.
        """
        from django.utils import timezone

        year       = timezone.now().year
        company_id = self.company_id  # Integer PK — always unique per company

        prefix = f"INV-{company_id}-{year}"

        for attempt in range(20):
            existing_count = Sale.objects.filter(
                company=self.company,
                sale_number__startswith=f"{prefix}-"
            ).count()
            candidate = f"{prefix}-{existing_count + 1 + attempt:06d}"
            if not Sale.objects.filter(sale_number=candidate).exists():
                return candidate

        # Absolute fallback: timestamp-based uniqueness
        import time
